...
// When a promotion occurs the details are written into *conv (if non-null).
string SemanticAnalyzer::resolveType(const string& t1, const string& t2,
    ...
    if (isNumericType(t1) && isNumericType(t2)) {
        string wider;
        if (t1 == "double" || t2 == "double") wider = "double";
        else if (t1 == "float"  || t2 == "float")  wider = "float";
        else wider = "int";

        // Record the promotion only when the types actually differ
        if (conv && t1 != t2) {
            // The narrower operand is the one that is NOT the wider type
            string narrower = (t1 != wider) ? t1 : t2;
            conv->fromType = narrower;
            conv->toType   = wider;
            conv->context  = t1 + " op " + t2;
        }
        return wider;
    }
...
string SemanticAnalyzer::visitExpr(shared_ptr<ASTNode> node) {
    ...
    if (t == "BinaryOp") {
        ...
        TypeConversion conv;
        conv.line = node->line;
        string resolved = resolveType(t1, t2, &conv);
        ...
        if (resolved == "mismatch") {
            ...
            addError("Type mismatch in expression: '" + t1 +
                     "' " + op + " '" + t2 + "'", *node);
            resolved = "unknown";
        } else if (!conv.fromType.empty()) {
            // A promotion occurred — build a readable context string
            string op = node->children[1]->value;
            conv.context = t1 + " " + op + " " + t2;
            conv.line    = node->line;
            node->conversion = conv;
    ...
    if (t == "+" || t == "-" ||
        ...
        if (resolved == "mismatch") {
            ...
            // A promotion occurred — record it
            conv.context = t1 + " " + t + " " + t2;
            conv.line    = node->line;
            node->conversion = conv;
            conversions_.push_back(conv);
        }
...
// ---------------------------------------------------------------------------
// Implicit type conversion log — output helpers
// ---------------------------------------------------------------------------

void SemanticAnalyzer::printConversions() const {
    if (conversions_.empty()) {
        cout << "[Type Conversions] No implicit conversions detected.\n";
        return;
    }
    ...
    for (const auto& c : conversions_) {
        cout << "  Line " << c.line
             << ": " << c.fromType << " \xe2\x86\x92 " << c.toType   // UTF-8 arrow →
             << "  (in expression: " << c.context << ")\n";
    }
    cout << "=================================\n\n";
}

void SemanticAnalyzer::writeConversions(const string& filePath) const {
    ofstream out(filePath);
    if (!out) {
        cerr << "[SemanticAnalyzer] Cannot open '" << filePath << "' for writing.\n";
        return;
    }

    out << "[\n";
    for (size_t i = 0; i < conversions_.size(); ++i) {
        const auto& c = conversions_[i];
        out << "  {\n"
            << "    \"line\": "      << c.line      << ",\n"
            << "    \"from\": \""    << c.fromType  << "\",\n"
            << "    \"to\": \""      << c.toType    << "\",\n"
            << "    \"context\": \"" << c.context   << "\"\n"
            << "  }" << (i + 1 < conversions_.size() ? "," : "") << "\n";
    }
    out << "]\n";

    cout << "[SemanticAnalyzer] Conversion log written to '" << filePath << "'.\n";
}
